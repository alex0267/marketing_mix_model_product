...
def extractSummary(responseModel, volumeContribution,ROS_Calculation, outputConfig):
    ...
    for item in outputConfig['CONTRIBUTORS']:
        ...
        #iterate through control variables and fill the touchpoint specific cells with 0
            ...
            spendings = pd.Series([0 for x in range(len(responseModel.controlFrame['YEAR_WEEK']))]).rename('spendings')
            ROS = pd.Series([0 for x in range(len(responseModel.controlFrame['YEAR_WEEK']))])

        tp_df = pd.concat([responseModel.controlFrame['YEAR_WEEK'],spendings,responseModel.target], ignore_index=False, axis =1)
