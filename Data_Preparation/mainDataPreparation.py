...
def run(configurations):
    ...
    seasonality_df = construct_seasonality_and_event_features(uniqueWeeks)
    ...
    promotion_df = compute_price_discount_feature(sellOut_df.copy(), quantile_reference=0.9)
    ...
    distribution_df = construct_distribution_feature(sell_out_distribution_df = sellOutDistribution_df,
    ...
    #create price_df
    price_df = calculatePrice(sellOut_df.copy(), configurations)
...
names = ["spending_df.csv", "seasonality_df.csv", "price_df.csv", "feature_df.csv", "control_df.csv", "targetRaw_df.csv", "indexColumns_df.csv"]
for i, elem in enumerate(run(configuration)): 
    elem.to_csv("output/" + names[i], index=False)
